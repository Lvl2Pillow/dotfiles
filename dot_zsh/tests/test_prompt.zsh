...
assert "$PROMPT" "%F{135}${clean}%f %F{076}main%f %(#.#.%%)%f " "clean repo: green branch"

# staged only → blue branch
run_precmd 0 "" "$clean" 1 0
assert "$PROMPT" "%F{135}${clean}%f %F{039}main%f %(#.#.%%)%f " "staged only: blue branch"

# unstaged only → yellow branch
run_precmd 0 "" "$clean" 0 1
assert "$PROMPT" "%F{135}${clean}%f %F{178}main%f %(#.#.%%)%f " "unstaged only: yellow branch"

# both staged and unstaged → unstaged wins (highest priority)
run_precmd 0 "" "$clean" 1 1
assert "$PROMPT" "%F{135}${clean}%f %F{178}main%f %(#.#.%%)%f " "staged+unstaged: yellow wins"
...
assert "$PROMPT" "%F{135}${detached}%f %F{076}@${sha:0:7}%f %(#.#.%%)%f " "detached HEAD: @<sha>"
...
assert "$PROMPT" "%F{135}${tight}%f %F{076}very-long-feature-branch-name-fo...poses%f %(#.#.%%)%f " "tight terminal: branch truncated"

# wide terminal (COLUMNS=120) with short dir+branch → no truncation, extra unused
run_precmd 0 "120" "$clean" 0 0
assert "$PROMPT" "%F{135}${clean}%f %F{076}main%f %(#.#.%%)%f " "wide terminal: no truncation"
