"""Debug: what does the async result contain when starting from HOME?"""
...
TRAPUSR1() {
  emulate -L zsh
  echo "TRAP_FIRED" >&2
  echo "BEFORE_PRECMD: staged=$_prompt_git_staged unstaged=$_prompt_git_unstaged untracked=$_prompt_git_untracked" >&2
  _prompt_rendering=1
  _prompt_precmd
  _prompt_rendering=0
  echo "AFTER_PRECMD: staged=$_prompt_git_staged unstaged=$_prompt_git_unstaged untracked=$_prompt_git_untracked" >&2
  _prompt_zle_append_footer
  zle .reset-prompt 2>/dev/null
  echo "TRAP_DONE" >&2
  echo "FOOTER=[$_prompt]" >&2
}
'''
...
for line in text.split('\n'):
    if any(x in line for x in ['TRAP_', 'staged=', 'FOOTER=']):
        clean = line.replace('\x1b', 'ESC')
        print(clean.strip()[:150])
